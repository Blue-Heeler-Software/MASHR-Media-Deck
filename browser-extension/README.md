...
This optional unpacked Chrome/Brave extension reads only the nine recommendation cards already visible beside the current `youtube.com/watch` video. It sends their video IDs and titles to MASHR Media Deck over loopback (`127.0.0.1`), then polls loopback for a video selected on the phone.
